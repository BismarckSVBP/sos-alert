...
import { MapPin, Navigation, Clock, Phone, Share, RefreshCw } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { useToast } from "@/hooks/use-toast";
...
const TrackLocation = () => {
  ...
  useEffect(() => {
    socket.emit("join-location", sessionId);
    socket.emit("new-tracker-viewer", { sessionId, viewerId: socket.id });

    socket.on("receive-location", ({ latitude, longitude, timestamp }) => {
      setIsSharing(true); // Ensure sharing remains "active" if location updates
      const pos = { lat: latitude, lng: longitude };
      setSenderPosition(pos);
      setLocationHistory((prev) => [...prev, pos]);
      setLastUpdate(new Date(timestamp || Date.now()));
  ...
  // -----> FIX 2: Only create DirectionsService when isLoaded && available
  useEffect(() => {
    if (
      isLoaded &&
      senderPosition &&
  ...
  }, [isLoaded, senderPosition, viewerPosition]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background p-4 text-center">
        <p className="text-lg font-semibold text-muted-foreground">
          Location sharing has ended, redirecting...
        </p>
      </div>
    );
  }
        ...
        <div className="container max-w-7xl mx-auto">
          ...
          <div className="grid lg:grid-cols-4 gap-6">
            ...
            <div className="space-y-4">
              ...
              >
                <RefreshCw className="mr-2" /> Refresh Tracker Page
              </Button>
