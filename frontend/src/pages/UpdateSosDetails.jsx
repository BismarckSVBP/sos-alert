...
//         // // ✅ Safely check if data is valid or redirect
//         // if (!data || Object.keys(data).length === 0) {
//         //   navigate("/sos-details-form", { replace: true });
//         //   return;
//         // }
//         const data = props.sosDetails;
...
//             emergencyMedicalInfo: data.personalInfo?.emergencyMedicalInfo || "",
//             dob: data.personalInfo?.dob || "",
//             passportPhotoUrl: data.personalInfo?.passportPhotoUrl || "",
//           },
//           emergencyContacts: data.emergencyContacts || [
//             { name: "", relation: "", phone: "", email: "", isPrimary: true },
//           ],
//           locationInfo: {
//             homeAddress: data.locationInfo?.homeAddress || "",
//             workAddress: data.locationInfo?.workAddress || "",
//             frequentLocations: data.locationInfo?.frequentLocations || "",
//           },
//           mobile: data.mobile || "",
...
//           const parsedDOB = new Date(data.personalInfo.dob);
//           setDob(parsedDOB);
//           calculateAge(parsedDOB);
//         }

//         if (data.personalInfo?.passportPhotoUrl) {
//           setProfileImage(data.personalInfo.passportPhotoUrl);
//         }
//       } catch (err) {
//         toast({
//           variant: "destructive",
//           title: "Failed to Load Data",
//           description: err.response?.data?.message || "Unable to fetch details",
//         });
//       }
//     };
...
//     setFormData((prev) => ({
//       ...prev,
//       personalInfo: { ...prev.personalInfo, age: age.toString() },
//     }));
//   };
...
//       await axios.put(
//         `${API_BASE_URL}/api/auth/update-sos-details`,
//         submissionData
//       );

//       toast({
//         title: "SOS Details Updated",
//         description: "Your emergency details have been updated successfully!",
//       });
...
//           {/* ⚙️ Insert all UI sections like before: image, personal info, emergency contacts, location info, buttons */}
//           {/* ✅ Reuse your existing form JSX */}
//           {/* ❗ Already correct above, so you can paste it here exactly as in previous messages */}

//           <form onSubmit={handleSubmit} className="space-y-8">
//             {/* Paste your full JSX form layout here */}
//             <Card>
//               <CardHeader>
...
// export default UpdateSosDetails;
...
const UpdateSosDetails = (props) => {
  ...
  const [formData, setFormData] = useState({
    ...
    emergencyContacts: [{ name: "", email: "", isPrimary: true }],
    locationInfo: {
      homeAddress: "",
    },
    mobile: "",
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (data.personalInfo?.dob) {
          setDob(new Date(data.personalInfo.dob));
        }
        if (data.personalInfo?.passportPhotoUrl) {
          setProfileImage(data.personalInfo.passportPhotoUrl);
        }
  ...
  const validateContacts = () => {
    const emails = formData.emergencyContacts.map((c) => c.email.trim()).filter(Boolean);
    const uniqueEmails = new Set(emails);
    if (uniqueEmails.size !== emails.length) {
      toast({
        variant: "destructive",
    ...
    try {
      ...
      const submissionData = {
        personalInfo: {
          ...
          bloodType: formData.personalInfo.bloodType,
          dob: dob ? dob.toISOString().split("T")[0] : "",
          passportPhotoUrl: photoUrl,
        },
        ...
        locationInfo: {
          homeAddress: formData.locationInfo.homeAddress,
        },
        mobile: formData.mobile,
