...
//         }
//       } catch (err) {
//         toast({ title: "Camera Permission Required", description: err.message, variant: "destructive" });
//       }
//     }
//     fetchCameras();
//   }, []);
...
//     });
//     socket.on("sender-disconnected", stopStreaming);
...
//     pc.onicecandidate = (e) => {
//       if (e.candidate) {
//         console.log("Sender ICE:", e.candidate);
//         socket.emit("candidate", { sessionId, targetId: viewerId, candidate: e.candidate });
//       }
//     };
//     return pc;
//   };
...
//       newStream.getTracks().forEach((track) => {
//         const sender = pc.getSenders().find((s) => s.track && s.track.kind === track.kind);
//         sender ? sender.replaceTrack(track) : pc.addTrack(track, newStream);
//       });
//     });
//   };
...
//         try { await videoRef.current.play(); } catch (e) { console.warn("Sender autoplay failed", e); }
//       }
//       localStreamRef.current = stream;
//       replaceTracks(stream);
//       await renegotiateAll();
...
//     <div className="min-h-screen flex items-center justify-center bg-background">
//       <Card className="w-full max-w-4xl">
//         <CardHeader><CardTitle className="text-xl">Emergency Media Sharing</CardTitle></CardHeader>
//         <CardContent className="relative">
//           <div className="aspect-video bg-black rounded-lg overflow-hidden">
...
//           <div className="mt-4 flex justify-center">
//             {cameras.length > 1 && (
//               <select className="border p-2 rounded-lg bg-background shadow" value={selectedCameraId} onChange={(e) => startStreaming(e.target.value)}>
//                 {cameras.map((c, i) => (
...
//           <div className="mt-4 flex gap-3 justify-center flex-wrap">
//             {!isStreaming ? (
//               <Button onClick={() => startStreaming(selectedCameraId)} className="bg-red-600 hover:bg-red-700" disabled={!selectedCameraId}><Camera className="mr-2 h-4 w-4" />Start Stream</Button>
//             ) : (
//               <>
//                 {!isPaused ? (
//                   <Button onClick={() => { localStreamRef.current?.getTracks().forEach((t) => (t.enabled = false)); setIsPaused(true); toast({ title: "Streaming paused" }); }} variant="outline"><Pause className="mr-2 h-4 w-4" />Pause Stream</Button>
//                 ) : (
//                   <Button onClick={() => { localStreamRef.current?.getTracks().forEach((t) => (t.enabled = true)); setIsPaused(false); toast({ title: "Streaming resumed" }); }} variant="outline"><Play className="mr-2 h-4 w-4" />Resume Stream</Button>
//                 )}
//                 <Button variant="outline" onClick={toggleAudio}>{isAudioEnabled ? <><Mic className="mr-2 h-4 w-4" />Mute</> : <><MicOff className="mr-2 h-4 w-4" />Unmute</>}</Button>
//                 <Button variant="outline" onClick={toggleVideo}><VideoOff className="mr-2 h-4 w-4" />{isVideoEnabled ? "Hide Video" : "Show Video"}</Button>
//                 <Button variant="destructive" onClick={stopStreaming}><Square className="mr-2 h-4 w-4" />Stop Stream</Button>
//               </>
//             )}
//           </div>
//         </CardContent>
