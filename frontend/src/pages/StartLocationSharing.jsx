...
const StartLocationSharing = () => {
  ...
  useEffect(() => {
    ...
    socket.on("new-tracker-viewer", ({ viewerId }) => {
      if (currentPosition) {
        socket.emit("send-location", {
          sessionId,
  ...
  const stopLocationSharing = () => {
    if (watchIdRef.current)
      navigator.geolocation.clearWatch(watchIdRef.current);
    socket.emit("terminate-location", { sessionId }); // <--- Trigger terminate-location for all in this session!
    setIsSharing(false);
    setIsPaused(false);
    setDuration(0);
    setLocationHistory([]);
